function [SM1,erM1,Vi1,pa_SM,outputCa]=M5con(z0,h1,ko1,D,P,Ta,Tm,Csn,SVC,A,b,K,f1,N,nn,t,Nx)
%%%R1G1M1 3参
...
for i=1:3
 if i==1 
    ...
    VM1(:,1)=var(reshape(outputCa(2:14,:,:),[13 N*N]),0,2 );
    ...
    Vpa1a(:,1)=mean(abs(VM1-var(outputCa(2:14,:,:),0,3)),2);    %mean(  var(outputCa(2:14,:,:),0,2),3 );
    rpa1a(:,1)=mean(abs(rM1-skewness(outputCa(2:14,:,:),0,3)),2);
    kpa1a(:,1)=mean(abs(kM1-kurtosis(outputCa(2:14,:,:),0,3)),2);
    Suba(:,1) = mean(  var(outputCa(2:14,:,:),0,2),3 );
